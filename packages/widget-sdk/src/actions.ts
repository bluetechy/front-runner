...
/*
 * Told about every click, whether or not the action did anything.
 *
 * Analytics is the next thing this package grows and this is the seam it will
 * grow into: today the host page may pass a function, and nothing in here
 * reports anywhere on its own. A widget that phoned home from inside a
 * customer's page without the customer having asked for it would be a widget
 * their privacy review removes.
 */
export interface WidgetEvent {
  /* "click" today. A view event belongs here too, and is not implemented:
   * see docs/TODO.md. */
  type: "click";
  /* The element's own id, which is one of the reasons ids in a definition are
   * stable and the author's. */
  nodeId: string;
  action: Action["type"];
  /* The event name on a `trackEvent`, and the properties it carried. */
  name?: string;
...
export interface ActionEnvironment {
  /* Where a navigation goes. Passed in rather than reached for, so a test
   * asserts what would have happened instead of the runner navigating. */
  navigate?: (url: string, newTab: boolean) => void;
  copy?: (text: string) => void;
  onEvent?: EventSink;
}

function navigateWith(url: string, newTab: boolean): void {
...
function copyWith(text: string): void {
...
export function performAction(
  ...
  switch (action.type) {
    case "navigate": {
      if (!isSafeUrl(action.url)) return;
      report?.({ type: "click", nodeId, action: "navigate" });
      (environment.navigate ?? navigateWith)(
        action.url.trim(),
        action.newTab ?? false,
      );
      return;

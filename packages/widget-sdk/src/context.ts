...
/* Keys the runtime itself fills in are computed per element and passed
 * alongside the host's: the progress bar's `{{remaining}}` is not something a
 * page could know. */
...
export function numberFrom(
  ...
  const raw =
    context && Object.hasOwn(context, name) ? context[name] : undefined;
...
export function withDefaults(
  variables: WidgetContext | undefined,
  context: WidgetContext | undefined,
): WidgetContext {
...
export function formatAmount(value: number, currency?: string): string {
  if (!currency) return String(Math.round(value * 100) / 100);
  try {

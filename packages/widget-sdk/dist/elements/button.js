...
export function ButtonElement({ node }) {
    const { mode, context, environment } = useRuntime();
    return (_jsx("button", { type: "button", "data-widget-node": node.id, onClick: () => performAction(node.action, node.id, environment), style: {
            font: "inherit",
            fontWeight: 600,
            cursor: "pointer",
        ...
        }, children: interpolate(node.label, context) }));
}

 ...
 * wrong.
 */
export function HotspotElement({ node }) {
    const { mode, environment } = useRuntime();
    return (_jsx("button", { type: "button", "data-widget-node": node.id, "aria-label": node.label, onClick: () => performAction(node.action, node.id, environment), style: {

 ...
 */
export function ProgressBarElement({ node }) {
    const { mode, context } = useRuntime();
    const value = node.source.type === "number"
        ? node.source.value
    ...
    const sentence = message
        ...
        : null;
    return (_jsxs("div", { "data-widget-node": node.id, style: {
            display: "flex",
        ...
        }, children: [sentence === null ? null : (_jsx("p", { style: { margin: 0, ...typeScale("body"), ...styleOf(node.style) }, children: sentence })), _jsx("div", { role: "progressbar", "aria-valuemin": 0, "aria-valuemax": node.goal, "aria-valuenow": Math.min(value, node.goal), "aria-valuetext": sentence ?? undefined, style: {

 ...
 */
export async function fetchWidget(endpoint, widgetId, options = {}) {
    ...
    try {
        ...
        throw new WidgetFetchError("The widget could not be fetched. If this page's origin is not on the widget's allowed origins, the browser will refuse the response without saying so.", 0);
    }
    if (!response.ok) {
    ...
    if (!document?.definition?.root)
        throw new WidgetFetchError("The API answered something that is not a widget.", response.status);
    return document;
}

...
const connection = {
    ...
    port: Number(process.env.PGPORT ?? process.env.POSTGRES_PUBLISHED_PORT ?? 30002),
    user: superUser,
...
const applySchema = async (client) => {
    ...
    if (rowCount > 0) {
        const permissions = (await readFile(path.join(sqlDir, 'Security', 'Permissions.sql'), 'utf8'))
...
await maintenance.query(`DROP DATABASE IF EXISTS "${testDatabase}" WITH (FORCE)`);
...
describe('main-db', () => {
    it('discovers test functions', () => {
        assert.ok(testNames.length > 0, 'no Test* functions were found in the test schema');
    });

...
async function build() {
  ...
  compiling = true;
  const compiler = spawn('tsc', ['-p', 'tsconfig.json'], { stdio: 'inherit', shell: process.platform === 'win32' });
  compiler.on('exit', async (code) => {
    if (code === 0) {
      if (server && server.exitCode === null) {
        await new Promise(resolve => { server.once('exit', resolve); server.kill('SIGTERM'); });
      }
      server = spawn(process.execPath, ['--env-file-if-exists=../../.env', 'dist/main.js'], { stdio: 'inherit' });
    }
    compiling = false;
...
let timer;
const watcher = watch('src', { recursive: true }, () => { clearTimeout(timer); timer = setTimeout(build, 150); });

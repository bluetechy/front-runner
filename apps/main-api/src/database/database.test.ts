...
describe("PostgreSQL adapter", () => {
  it("passes bound values through and closes its pool on shutdown", async () => {
    const query = jest
      ...
      .mockImplementation((async () => ({ rows: [{ count: 1 }] })) as never);
    const end = jest
      .spyOn(Pool.prototype, "end")
      .mockImplementation((async () => undefined) as never);
    const service = new DatabaseService(config);
    await expect(service.query("SELECT $1 AS count", [1])).resolves.toEqual([
      { count: 1 },
    ]);
  ...
  it.each([
    ...
    "surfaces the deliberate exception %s as a client error",
    async (message, _name, status) => {
      jest.spyOn(Pool.prototype, "query").mockImplementation((async () => {
  ...
  it("sanitizes database errors and logs only the error code", async () => {
    jest.spyOn(Pool.prototype, "query").mockImplementation((async () => {
      throw Object.assign(new Error("private SQL and credentials"), {
        code: "42501",
      });
    }) as never);
    const log = jest
    ...
    const service = new DatabaseService(config);
    try {
      await expect(service.query("SELECT sensitive_data")).rejects.toThrow(

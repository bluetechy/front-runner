...
export class OrganizationsService {
  ...
  async get(loginName: string, organizationId: string) {
    const [organization] = await this.db.query<Organization>('SELECT * FROM dbo."GetOrganization"($1, $2)', [loginName, organizationId]);
    return organization ?? null;
  }

  async setEnabled(loginName: string, organizationId: string, isEnabled: boolean) {
    const [organization] = await this.db.query<Organization>(
      'SELECT * FROM dbo."SetOrganizationEnabled"($1, $2, $3)', [loginName, organizationId, isEnabled],
    );
    return organization ?? null;
  }
  ...
  // disagree with it.
  async setRole(loginName: string, organizationId: string, userId: string, isOwner: boolean) {
    const [member] = await this.db.query<OrganizationMember>(
      'SELECT * FROM dbo."SetOrganizationRole"($1, $2, $3, $4)', [loginName, organizationId, userId, isOwner],
    );
    return member ?? null;
  }
  ...
  async rename(loginName: string, organizationId: string, name: string) {
    const [organization] = await this.db.query<Organization>(
      'SELECT * FROM dbo."RenameOrganization"($1, $2, $3)', [loginName, organizationId, name],
    );
    return organization ?? null;
  }

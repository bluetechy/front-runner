...
const account = { UserUUID: userId, Name: 'Alice Example', LoginName: 'alice', Email: 'alice@example.test', IsEnabled: true };
const profile = { UserUUID: userId, Name: 'Alice Example', LoginName: 'alice', Email: 'alice@example.test', IsAdmin: false };
const organization = { OrganizationUUID: orgId, Name: 'Acme', TeamCount: 1, UserCount: 1, OwnerCount: 1, IsOwner: true, IsEnabled: true };
const team = { OrganizationUUID: orgId, TeamUUID: teamId, Name: 'Builders', UserCount: 1, IsManager: true };
const member = { UserUUID: userId, Name: 'Alice Example', LoginName: 'alice', Email: 'alice@example.test', IsOwner: true, JoinedAt: new Date('2026-01-01T00:00:00Z') };
const invitation = {
  InvitationUUID: invitationId, OrganizationUUID: orgId, OrganizationName: 'Acme', Email: 'newcomer@example.test',
  IsOwner: false, Status: 'Pending', ExpiresAt: new Date('2030-01-01T00:00:00Z'), RespondedAt: null, InvitedByLoginName: 'alice',
};
...
describe('GraphQL application', () => {
  ...
  const sign = async (claims: Record<string, unknown> = {}) => new SignJWT({
    typ: 'Bearer', preferred_username: 'alice', name: 'Alice Example', email: 'alice@example.test', ...claims,
  })
    .setProtectedHeader({ alg: 'RS256' }).setIssuedAt().setSubject('subject-alice')
    .setIssuer(issuer).setAudience('main-api').setExpirationTime('1h')
    .sign(signingKey);
  ...
  beforeAll(async () => {
    ...
    const module = await Test.createTestingModule({ imports: [AppModule] })
      .overrideProvider(DatabaseService).useValue({ query })
      .overrideProvider(KEYCLOAK_KEY_SET).useValue(createLocalJWKSet({ keys: [{ ...jwk, alg: 'RS256', use: 'sig' }] }))
      .compile();
  ...
  beforeEach(() => {
    ...
    query.mockImplementation(async sql => {
      ...
      if (sql.includes('"GetUser"') || sql.includes('"GetUsers"')) return [profile];
      ...
      if (/"(GetOrganizations|GetOrganization|AddOrganization|LeaveOrganization|RenameOrganization|SetOrganizationEnabled)"/.test(sql)) return [organization];
      if (/"(GetOrganizationMembers|SetOrganizationRole)"/.test(sql)) return [member];
  ...
  it('serves all six read features through one schema and preserves exact decimals and ISO timestamps', async () => {
    ...
    expect(query.mock.calls.filter(([sql]) => sql.includes('WHERE "SubjectId"'))).toHaveLength(1);
  });
  ...
  it.each([
    ...
    ['inviteToOrganization', `organizationId: "${orgId}", email: "Newcomer@Example.test"`, 'InvitationUUID', 'InviteToOrganization', ['alice', orgId, 'newcomer@example.test', false]],
    ['acceptInvitation', `invitationId: "${invitationId}"`, 'InvitationUUID', 'AcceptOrganizationInvitation', ['alice', invitationId]],
    ['declineInvitation', `invitationId: "${invitationId}"`, 'InvitationUUID', 'DeclineOrganizationInvitation', ['alice', invitationId]],
    ['revokeInvitation', `invitationId: "${invitationId}"`, 'InvitationUUID', 'RevokeOrganizationInvitation', ['alice', invitationId]],
    ['renameOrganization', `organizationId: "${orgId}", name: "  Acme Holdings  "`, 'Name', 'RenameOrganization', ['alice', orgId, 'Acme Holdings']],
    ['setOrganizationRole', `organizationId: "${orgId}", userId: "${userId}", isOwner: true`, 'IsOwner', 'SetOrganizationRole', ['alice', orgId, userId, true]],
    ['setOrganizationEnabled', `organizationId: "${orgId}", isEnabled: false`, 'IsEnabled', 'SetOrganizationEnabled', ['alice', orgId, false]],
  ...
  it('no longer exposes a way to add a user to an organization directly', async () => {
    const response = await execute(`mutation { joinOrganization(organizationId: "${orgId}", userId: "${userId}") { Name } }`);
  ...
  it('reads invitations from both sides', async () => {
    const response = await execute(`query($org: String!) {
    ...
    expect(response.body.errors).toBeUndefined();
    expect(response.body.data.invitations[0]).toMatchObject({ OrganizationName: 'Acme', Status: 'Pending', ExpiresAt: '2030-01-01T00:00:00.000Z' });
    expect(response.body.data.organizationInvitations[0].RespondedAt).toBeNull();
    expect(query).toHaveBeenCalledWith(expect.stringContaining('dbo."GetUserInvitations"'), ['alice', 50, 0]);
    expect(query).toHaveBeenCalledWith(expect.stringContaining('dbo."GetOrganizationInvitations"'), ['alice', orgId, 50, 0]);
  });

  it('reads one organization on its own', async () => {
    const response = await execute(`query($org: String!){ organization(organizationId: $org){ Name IsOwner IsEnabled } }`, { org: orgId });
    expect(response.body.errors).toBeUndefined();
    expect(response.body.data.organization).toEqual({ Name: 'Acme', IsOwner: true, IsEnabled: true });
    expect(query).toHaveBeenCalledWith(expect.stringContaining('dbo."GetOrganization"'), ['alice', orgId]);
  });
  ...
  it('answers null for an organization the caller does not belong to', async () => {
    query.mockImplementation(async sql => (sql.includes('WHERE "SubjectId"') ? [account] : []));
    const response = await execute(`query($org: String!){ organization(organizationId: $org){ Name } }`, { org: orgId });
    expect(response.body.errors).toBeUndefined();
  ...
  it('hides archived organizations from the list unless asked for them, and says which are archived', async () => {
    ...
    expect(query).toHaveBeenCalledWith(expect.stringContaining('dbo."GetOrganizations"'), ['alice', 50, 0, false]);

    query.mockImplementation(async sql => (sql.includes('WHERE "SubjectId"') ? [account] : [{ ...organization, IsEnabled: false }]));
    const response = await execute('{ organizations(includeArchived: true) { Name IsEnabled } }');
    expect(query).toHaveBeenCalledWith(expect.stringContaining('dbo."GetOrganizations"'), ['alice', 50, 0, true]);
    expect(response.body.data.organizations[0]).toEqual({ Name: 'Acme', IsEnabled: false });
  });

  it('lists the people in an organization, owners first', async () => {
    const response = await execute(`query($org: String!){ organizationMembers(organizationId: $org){ LoginName IsOwner JoinedAt } }`, { org: orgId });
    expect(response.body.errors).toBeUndefined();
    expect(response.body.data.organizationMembers[0]).toEqual({ LoginName: 'alice', IsOwner: true, JoinedAt: '2026-01-01T00:00:00.000Z' });
    expect(query).toHaveBeenCalledWith(expect.stringContaining('dbo."GetOrganizationMembers"'), ['alice', orgId, 50, 0]);
  });
  ...
  it.each([
    ['setOrganizationRole', `mutation { setOrganizationRole(organizationId: "${orgId}", userId: "${userId}", isOwner: false) { IsOwner } }`,
      new BadRequestException('The last owner cannot be demoted.'), 'BAD_REQUEST', 'The last owner cannot be demoted.'],
    ['organizationInvitations', `query { organizationInvitations(organizationId: "${orgId}") { Email } }`,
      new ForbiddenException('Action cannot be performed.'), 'FORBIDDEN', 'Action cannot be performed.'],
    ['acceptInvitation', `mutation { acceptInvitation(invitationId: "${invitationId}") { Status } }`,
      new BadRequestException('That invitation has expired.'), 'BAD_REQUEST', 'That invitation has expired.'],
  ])('reports a rule the database refused on %s without turning it into a failure', async (_operation, document, thrown, code, message) => {
    query.mockImplementation(async sql => {
      if (sql.includes('WHERE "SubjectId"')) return [account];
      throw thrown;
    });
  ...
  it('rejects a blank organization name on rename', async () => {
    const response = await execute(`mutation { renameOrganization(organizationId: "${orgId}", name: "   ") { Name } }`);
    expect(response.body.errors[0].extensions.code).toBe('BAD_REQUEST');
    expect(query.mock.calls.some(([sql]) => sql.includes('RenameOrganization'))).toBe(false);
  });

  it('rejects an invitation to an address it could not store', async () => {
    const response = await execute(`mutation { inviteToOrganization(organizationId: "${orgId}", email: "not-an-address") { Email } }`);
    expect(response.body.errors[0].extensions.code).toBe('BAD_REQUEST');
    expect(query.mock.calls.some(([sql]) => sql.includes('InviteToOrganization'))).toBe(false);
  });
  ...
  it('provisions an account the first time a verified subject appears', async () => {
    query.mockImplementation(async sql => (sql.includes('WHERE "SubjectId"') ? [] : sql.includes('"ProvisionUser"') ? [account] : [profile]));
    const response = await execute('{ me { LoginName } }');
    expect(response.body.errors).toBeUndefined();
    expect(query).toHaveBeenCalledWith(expect.stringContaining('dbo."ProvisionUser"'), ['subject-alice', 'alice', 'Alice Example', 'alice@example.test']);
  });
  ...
  it('refreshes a renamed account, and writes nothing when nothing changed', async () => {
    ...
    expect(query.mock.calls.some(([sql]) => sql.includes('ProvisionUser'))).toBe(false);

    const renamed = await sign({ preferred_username: 'alice.example' });
    await execute('{ me { LoginName } }', {}, `Bearer ${renamed}`);
    expect(query).toHaveBeenCalledWith(expect.stringContaining('dbo."ProvisionUser"'), ['subject-alice', 'alice.example', 'Alice Example', 'alice@example.test']);
  });
  ...
  it('rejects a token signed by anything but the realm', async () => {
    const impostor = (await generateKeyPair('RS256', { extractable: true })).privateKey;
    const forged = await new SignJWT({ typ: 'Bearer', preferred_username: 'alice' })
      .setProtectedHeader({ alg: 'RS256' }).setIssuedAt().setSubject('subject-alice')
      .setIssuer(issuer).setAudience('main-api').setExpirationTime('1h').sign(impostor);
  ...
  it('passes pagination to PostgreSQL', async () => {
    ...
    expect(response.body.errors).toBeUndefined();
    expect(query).toHaveBeenCalledWith(expect.stringContaining('LIMIT $2 OFFSET $3'), ['alice', 10, 20, false]);
  });
  ...
  it('denies team creation without organization ownership', async () => {
    query.mockImplementation(async sql => sql.includes('WHERE "SubjectId"') ? [account] : [{ allowed: false }]);
  ...
  it('requires a token on every operation the schema exposes', async () => {
    ...
    const roots = [schema.getQueryType(), schema.getMutationType()].filter(root => !!root);
    const placeholder = (type: unknown) => {
      const named = String(type).replace(/[[\]!]/g, '');
      return named === 'Boolean' ? 'false' : named === 'Int' ? '1' : `"${orgId}"`;
    };
    const operations = roots.flatMap(root => Object.values(root.getFields()).map(field => {
      const args = field.args
        .filter(argument => String(argument.type).endsWith('!') && argument.defaultValue === undefined)
        .map(argument => `${argument.name}: ${placeholder(argument.type)}`);
      const selection = `${field.name}${args.length ? `(${args.join(', ')})` : ''}`;
      return { name: field.name, document: `${root.name.toLowerCase()} { ${selection} { __typename } }` };
    }));
    ...
    for (const { name, document } of operations) {
      const response = await execute(document, {}, '');
      expect({ name, code: response.body.errors?.[0]?.extensions?.code }).toEqual({ name, code: 'UNAUTHENTICATED' });
      // A refusal that still resolved the field would report both.
      expect({ name, value: response.body.data?.[name] ?? null }).toEqual({ name, value: null });
    }

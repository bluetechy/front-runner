...
import { CurrentUser, Public } from "./authentication.decorators.js";
import {
  KEYCLOAK_KEY_SET,
  KeycloakService,
 ...
 * worth stating in one place: two decorators for writing resolvers, the
 * module, and the pieces the tests need to stand a realm up without a
 * network. `Principal`, `GraphqlContext` and `VerifiedIdentity` are types and
 * leave nothing behind at runtime, which is why they are not in this list.
 */

describe("what authentication offers the rest of the API", () => {
  it("offers its module, its service, its key set, and the two decorators", () => {

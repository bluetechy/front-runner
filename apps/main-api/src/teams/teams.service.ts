...
export class TeamsService {
  ...
  list(loginName: string, organizationId: string, page: PageArgs) {
    ...
    return this.db.query<Team>('SELECT DISTINCT * FROM dbo."GetTeams"($1, $2) ORDER BY "Name", "TeamUUID" LIMIT $3 OFFSET $4', [loginName, organizationId, page.limit, page.offset]);
  }
  async add(loginName: string, organizationId: string, name: string) {
    const [access] = await this.db.query<{ allowed: boolean }>('SELECT dbo."IsOwnerOfOrganization"($1, $2) AS allowed', [loginName, organizationId]);
    if (!access?.allowed) throw new ForbiddenException('Organization owner access is required');
    const [team] = await this.db.query<Team>('SELECT * FROM dbo."AddTeam"($1, $2, $3)', [loginName, organizationId, name]);
    return team ?? null;
  }
  private async access(user: Principal, teamId: string, userId: string, leaving: boolean) {
    const [access] = await this.db.query<{ member: boolean; manager: boolean; target_member: boolean }>(`
    ...
    if (!access?.member || (!access.manager && !(leaving && user.userId === userId)) || (!leaving && !access.target_member)) {
  ...
  async join(user: Principal, teamId: string, userId: string) {
    ...
    const [team] = await this.db.query<Team>(`SELECT * FROM dbo."JoinTeam"($1, $2, $3,
      COALESCE((SELECT "IsManager" FROM dbo."UserTeams" WHERE "TeamUUID" = $2 AND "UserUUID" = $3), false))`, [user.loginName, teamId, userId]);
    return team ?? null;
  }
  async leave(user: Principal, teamId: string, userId: string) {
    await this.access(user, teamId, userId, true);
    const [team] = await this.db.query<Team>('SELECT * FROM dbo."LeaveTeam"($1, $2, $3)', [user.loginName, teamId, userId]);
    return team ?? null;
  }
}

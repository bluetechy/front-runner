...
export const queryLimits: ValidationRule = (context) => ({
  OperationDefinition(operation) {
    let fields = 0;
    const walk = (
      set: typeof operation.selectionSet,
      depth: number,
      ...
      for (const selection of set.selections) {
        if (selection.kind === Kind.FIELD) {
          ...
          if (
            ...
            !walk(selection.selectionSet, depth + 1, fragments)
          )
            return false;
          ...
          if (
            fragment &&
            !walk(fragment.selectionSet, depth, new Set([...fragments, name]))
          )
            return false;
        }
      }
    ...
    if (!walk(operation.selectionSet, 1, new Set())) {
      context.reportError(
        new GraphQLError("Query exceeds the depth or field limit", {

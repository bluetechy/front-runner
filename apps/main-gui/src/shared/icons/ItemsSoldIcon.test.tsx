...
import { ItemsSoldIcon } from './ItemsSoldIcon';

describe('icons/ItemsSoldIcon', () => {

	test('it renders without errors', () => {
		render(<ItemsSoldIcon />);
	});
	...
	test('it renders with custom props', () => {
		...
		const { container } = render(<ItemsSoldIcon color={color} size={size} />);
	...
	test('it takes a keyword colour as well as a hex one', () => {
		const { container } = render(<ItemsSoldIcon color="currentColor" />);
		const svgTag = container.querySelector<HTMLElement>('svg');

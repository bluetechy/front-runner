...
describe('icons/AchievementsIcon', () => {

	test('it renders without errors', () => {
		render(<AchievementsIcon />);
	});
	...
	test('it renders with custom props', () => {
		...
		const { container } = render(<AchievementsIcon color={color} size={size} />);
	...
	test('it takes a keyword colour as well as a hex one', () => {
		const { container } = render(<AchievementsIcon color="currentColor" />);
		const svgTag = container.querySelector<HTMLElement>('svg');

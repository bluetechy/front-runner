...
describe('icons/ProcessingIcon', () => {

	test('it renders without errors', () => {
		render(<ProcessingIcon />);
	});
	...
	test('it renders with custom props', () => {
		...
		const { container } = render(<ProcessingIcon color={color} size={size} />);
	...
	test('it takes a keyword colour as well as a hex one', () => {
		const { container } = render(<ProcessingIcon color="currentColor" />);
		const svgTag = container.querySelector<HTMLElement>('svg');

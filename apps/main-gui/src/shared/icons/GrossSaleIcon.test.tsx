...
import { GrossSaleIcon } from './GrossSaleIcon';

describe('icons/GrossSaleIcon', () => {

	test('it renders without errors', () => {
		render(<GrossSaleIcon />);
	});
	...
	test('it renders with custom props', () => {
		...
		const { container } = render(<GrossSaleIcon color={color} size={size} />);
	...
	test('it takes a keyword colour as well as a hex one', () => {
		const { container } = render(<GrossSaleIcon color="currentColor" />);
		const svgTag = container.querySelector<HTMLElement>('svg');
